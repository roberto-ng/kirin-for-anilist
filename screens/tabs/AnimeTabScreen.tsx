...
export default function AnimeTabScreen(): JSX.Element {
    ...
    const fetchInitialData = (token: string, user: User): void => {
        setIsLoading(true);
        fetchMediaList(token, user.id, MediaType.ANIME, MediaListStatus.CURRENT, 1)
            .then((page) => {
                setIsLoading(false);
    ...
    const handleEndReached = async (): Promise<void> => {
        ...
        try {
            setIsLoading(true);
            const page = await fetchMediaList(token, user.id, MediaType.ANIME, nextSection, pageNumber);
            setIsLoading(false);
